...
class MaterialSlider(context: Context?, attrs: AttributeSet?) : androidx.appcompat.widget.AppCompatSeekBar(context, attrs) {
    ...
    init {
        ...
        context?.obtainStyledAttributes(attrs, R.styleable.MaterialSlider)?.let {
            val thumbColor = it.getColor(
                R.styleable.MaterialSlider_thumbColour,
                R.color.mid_green.getColour(context)
            )
            thumb.setColorFilter(thumbColor, PorterDuff.Mode.SRC_IN)
            tickBeforePaint.color = it.getColor(
                R.styleable.MaterialSlider_tickSelectedColour,
                R.color.mid_green.getColour(context)
            )
            tickAfterPaint.color = it.getColor(
                R.styleable.MaterialSlider_tickColour,
                R.color.mid_green.getColour(context)
            )
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (i in 0..sections) {
            val position =
                (increment * i) + paddingLeft //adding padding left because 0 is actually 0 + padding left
            if (thumb.bounds.right in position - 10..position + 10) continue
            canvas?.drawCircle(
                position, height.toFloat() / 2, 1.convertToPixels(context).toFloat(),
...
fun Int.getColour(context: Context?) =
    context?.let { ResourcesCompat.getColor(it.resources, this, null) } ?: 0
...
fun Int.convertToPixels(context: Context?) = context?.let { resContext ->
    Math.round(
        TypedValue.applyDimension(
            ...
            this.toFloat(),
            resContext.resources.displayMetrics
        )
    )
        .takeIf { it > 1 } ?: 1
} ?: 0